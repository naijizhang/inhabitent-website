 ...
 */
function inhabitent_comment_count()
{
 ...
 */
function inhabitent_entry_footer()
{
 	// Hide category and tag text for pages.
	if ('post' === get_post_type()) {
 		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list(esc_html(', '));
		if ($categories_list && inhabitent_categorized_blog()) {
 ...
 * @return bool
 */
function inhabitent_categorized_blog()
{
	if (false === ($all_the_cool_cats = get_transient('inhabitent_categories'))) {
...
function inhabitent_category_transient_flusher()
{
	if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
 	...
 	// Like, beat it. Dig?
	delete_transient('inhabitent_categories');
}
add_action('edit_category', 'inhabitent_category_transient_flusher');
add_action('save_post', 'inhabitent_category_transient_flusher');
 ...
 */
function inhabitent_comment_list($comment, $args, $depth)
{
 ...
 */
function inhabitent_numbered_pagination()
{
	global $wp_query;
	$big = 999999999;
